...
export function computeFilteredMarkersAndExpression(
    ...
    const filtered: TPopups = Object.fromEntries(
        Object.entries(popups).filter(([markerId, categoryPayloads]) => {
            const hasActiveCategory = Object.keys(categoryPayloads).some(
                (cat) => activeSet.has(cat as TCategory)
            );
            const isBookmarked =
                !bookmarkedIdSet || bookmarkedIdSet.has(markerId);
